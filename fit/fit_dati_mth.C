...
// Set a bunch of parameters to make the plot look nice
 ...
 // c1->UseCurrentStyle();
  ...
  // Import DATA  
  RooDataSet* data_pp = new RooDataSet("data_pp","data_pp",RooArgSet(mth,mll), Cut(mll_cut), Import(*tree_data_pp));
  RooDataSet* data_fp = new RooDataSet("data_fp","data_fp",RooArgSet(mth,mll), Cut(mll_cut), Import(*tree_data_fp));
 
  // Combine PassProbe and FailProbe datasets
  RooDataSet combData("combData","combined data", mth, Index(passprobe_cat),Import("pass",*data_pp),Import("fail",*data_fp)) ;
 

  // Import TTbar and Bkg datasets
  RooDataSet* ttbar_pp = new RooDataSet("ttbar_pp", "ttbar_pp", RooArgSet(mth, mll, weight), Cut(mll_cut), Import(*tree_ttbar_pp), WeightVar(weight));
  RooDataSet* ttbar_fp = new RooDataSet("ttbar_fp", "ttbar_fp", RooArgSet(mth, mll, weight), Cut(mll_cut), Import(*tree_ttbar_fp), WeightVar(weight));
  
  RooDataSet* otherbkg_pp = new RooDataSet("otherbkg_pp", "otherbkg_pp", RooArgSet(mth, mll, weight), Cut(mll_cut), Import(*tree_bkg_pp), WeightVar(weight));
  RooDataSet* otherbkg_fp = new RooDataSet("otherbkg_fp", "otherbkg_fp", RooArgSet(mth, mll, weight), Cut(mll_cut), Import(*tree_bkg_fp), WeightVar(weight));
