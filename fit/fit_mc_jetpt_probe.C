...
// Set a bunch of parameters to make the plot look nice
 ...
 // c1->UseCurrentStyle();
  ...
  RooDataSet combMC("combMC","combined MC", RooArgSet(jetpt_probe,weight),Index(passprobe_cat),Import("pass",*ttbar_pp_mc),Import("fail",*ttbar_fp_mc),WeightVar(weight)) ;
...
/*
  ...
  RooMCStudy mc(total_fit, RooArgSet(jetpt_probe,passprobe_cat), FitModel(total_fit), Extended(true), FitOptions(Save(true),Minos(RooArgSet(efficiency_s))), FitOptions(NumCPU(4,0)));
...
//      cout << "Efficiency s = " << efficiency_s.getValV() << "+-" << efficiency_s.getError() << endl;
  ...
  h_pull_effs->Draw();
*/
  RooPlot* frame = jetpt_probe.frame(Title("fit_pass.C"),Bins(nbins),Range(30,100,1));
  RooPlot* frame1 = jetpt_probe.frame(Title("fit_fail.C"),Bins(nbins),Range(30,100,1));
