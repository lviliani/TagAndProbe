...
// Set a bunch of parameters to make the plot look nice
 ...
 // c1->UseCurrentStyle();
  ...
  TFile* fileout = new TFile("mc_jetptprobe_above30.root", "RECREATE");
  /*
...
*/
  ...
  for (int i=0; i<ntoys; i++){
    ...
    /* 
    RooDataSet* toy_comb = total_fit.generate(RooArgSet(jetptprobe, passprobe_cat), NumEvents(int(entries_tt_pp+entries_tt_fp+entries_bkg_pp+entries_bkg_fp)), AutoBinned(false));
    comb_mc = toy_comb;
    comb_mc->SetName("ttbar_pp");
    */
...
//  mc.genData(0)->plotOn(frame,Cut("passprobe==passprobe::pass")) ;
  total_fit.plotOn(frame,Slice(passprobe_cat,"pass"),ProjWData(passprobe_cat,*comb_mc), Binning(binning)) ;
  total_fit.plotOn(frame,Slice(passprobe_cat,"pass"),Components("pdf_otherbkg_pp"),ProjWData(passprobe_cat,*comb_mc),LineStyle(kDashed), Binning(binning)) ;
  total_fit.plotOn(frame,Slice(passprobe_cat,"pass"),Components("pdf_ttbar_pp"),ProjWData(passprobe_cat,*comb_mc),LineStyle(kDashed),LineColor(kRed), Binning(binning));
  comb_mc->plotOn(frame,Cut("passfail==passfail::pass"), Binning(binning)) ;
  ttbar_pp->plotOn(frame,MarkerColor(kRed), LineWidth(0), Binning(binning));
  otherbkg_pp->plotOn(frame,MarkerColor(kBlue), LineWidth(0), Binning(binning));
...
//  mc.genData(0)->plotOn(frame1,Cut("passfail==passfail::fail")) ;
  ...
  total_fit.plotOn(frame1,Slice(passprobe_cat,"fail"),ProjWData(passprobe_cat,*comb_mc), Binning(binning)) ;
  total_fit.plotOn(frame1,Slice(passprobe_cat,"fail"),Components("pdf_otherbkg_fp"),ProjWData(passprobe_cat,*comb_mc),LineStyle(kDashed), Binning(binning)) ;
  total_fit.plotOn(frame1,Slice(passprobe_cat,"fail"),Components("pdf_ttbar_fp"),ProjWData(passprobe_cat,*comb_mc),LineStyle(kDashed),LineColor(kRed), Binning(binning)) ;
  comb_mc->plotOn(frame1,Cut("passfail==passfail::fail"), Binning(binning)) ;
  ttbar_fp->plotOn(frame1,MarkerColor(kRed), LineWidth(0), Binning(binning));
  otherbkg_fp->plotOn(frame1,MarkerColor(kBlue), LineWidth(0), Binning(binning));
  frame1->Print();
