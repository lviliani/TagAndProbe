...
// Set a bunch of parameters to make the plot look nice
 ...
 // c1->UseCurrentStyle();
  ...
  TString mll_cut = "mll>0 && mth>0 && mth<200";
  ...
  RooRealVar mth("mth", "mth", 0, 200);
  RooRealVar weight("weight", "weight", -1000, 1000);  
  RooRealVar mll("mll", "mll", 50, 200);
  ...
  RooDataSet* ttbar_pp = new RooDataSet("ttbar_pp", "ttbar_pp", RooArgSet(mth, weight, mll), Import(*tree_ttbar_pp), WeightVar(weight), Cut(mll_cut));
  RooDataSet* ttbar_fp = new RooDataSet("ttbar_fp", "ttbar_fp", RooArgSet(mth, weight, mll), Import(*tree_ttbar_fp), WeightVar(weight), Cut(mll_cut));
  RooDataSet* otherbkg_pp = new RooDataSet("otherbkg_pp", "otherbkg_pp", RooArgSet(mth, weight, mll), Import(*tree_bkg_pp), WeightVar(weight), Cut(mll_cut));
  RooDataSet* otherbkg_fp = new RooDataSet("otherbkg_fp", "otherbkg_fp", RooArgSet(mth, weight, mll), Import(*tree_bkg_fp), WeightVar(weight), Cut(mll_cut));
  ...
  RooDataHist dttbar_pp("dttbar_pp","ttbar_pp",RooArgSet(mth),*ttbar_pp) ;
  RooDataHist dttbar_fp("dttbar_fp","ttbar_fp",RooArgSet(mth),*ttbar_fp) ;
  RooDataHist dotherbkg_pp("dotherbkg_pp","otherbkg_pp",RooArgSet(mth),*otherbkg_pp) ;
  RooDataHist dotherbkg_fp("dotherbkg_fp","otherbkg_fp",RooArgSet(mth),*otherbkg_fp) ;


  RooHistPdf pdf_ttbar_pp("pdf_ttbar_pp","ttbar_pp",RooArgSet(mth),dttbar_pp);
  RooHistPdf pdf_ttbar_fp("pdf_ttbar_fp","ttbar_fp",RooArgSet(mth),dttbar_fp);
  RooHistPdf pdf_otherbkg_pp("pdf_otherbkg_pp","otherbkg_pp",RooArgSet(mth),dotherbkg_pp);
  RooHistPdf pdf_otherbkg_fp("pdf_otherbkg_fp","otherbkg_fp",RooArgSet(mth),dotherbkg_fp);
  ...
  RooDataSet combMC("combMC","combined MC", RooArgSet(mth,weight),Index(passprobe_cat),Import("pass",*ttbar_pp_mc),Import("fail",*ttbar_fp_mc),WeightVar(weight)) ;
...
/*
  ...
  RooMCStudy mc(total_fit, RooArgSet(mth,passprobe_cat), FitModel(total_fit), Extended(true), FitOptions(Save(true),Minos(RooArgSet(efficiency_s))), FitOptions(NumCPU(4,0)));
...
*/
  RooPlot* frame = mth.frame(Title("Pass Probe Events"),Bins(nbins),Range(0,200,1));
  RooPlot* frame1 = mth.frame(Title("Fail Probe Events"),Bins(nbins),Range(0,200,1));
